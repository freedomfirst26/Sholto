...
/// <summary>
/// Forwards to whatever <see cref="IAnalysisCache"/> is currently attached — a
/// permanent miss until <see cref="Attach"/> is called once. Same swap pattern as
/// <see cref="SwitchableKeyAnalysisCache"/>, applied to the DB tier of
...
/// </summary>
public sealed class SwitchableAnalysisCache : IAnalysisCache
{
    private IAnalysisCache? _target;

    public string Name => _target?.Name ?? "unattached";

    /// <summary>Point at the real (DB-backed) cache. Call once, after the DB opens.</summary>
    public void Attach(IAnalysisCache real) => _target = real;
